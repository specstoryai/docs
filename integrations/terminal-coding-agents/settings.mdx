...
| `run`     | Launch Claude Code, Cursor CLI, Codex CLI or Gemini CLI with autosave to local Markdown (and SpecStory Cloud if logged in) | `specstory run claude`<br />`specstory run cursor`<br />`specstory run codex`<br />`specstory run gemini` |
