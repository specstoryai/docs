...
<Tabs>
  ...
  <Tab title="Claude Code + Cursor CLI">
    ...
    | `run`     | Run Claude Code or Cursor CLI with auto-save to markdown    | `specstory run claude`<br />`specstory run cursor`    |
    | `sync`    | Convert existing sessions to markdown                       | `specstory sync`<br />`specstory sync -s <sessionId>` |
