...
<Tabs>
  ...
  <Tab title="Terminal Coding Agents">
    ...
    | `run`     | Run Claude Code, Cursor CLI or Codex CLI with auto-save to to local markdown files, and to SpecStory Cloud if logged in | `specstory run claude`<br />`specstory run cursor`<br />`specstory run codex` |
    | `sync`    | Convert existing sessions to local markdown files, and sync them to SpecStory Cloud if logged in                        | `specstory sync`<br />`specstory sync -s <sessionId>`                         |
    | `check`   | Terminal agent(s) are installed and accessible by SpecStory                                                             | `specstory check`                                                             |
    | `login`   | Login to SpecStory Cloud                                                                                                | `specstory login`                                                             |
    | `logout`  | Logout of SpecStory Cloud                                                                                               | `specstory logout`                                                            |
    ...
    | ---------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------- |
    | `-c, --command`        | Custom Claude Code command/args                                 | `specstory run claude -c "~/.claude/local/claude --dangerously-skip-permissions"` |
    | `-s, --session`        | Sync a specific session to markdown                             | `specstory sync claude -s <sessionId>`                                            |
    | `--console`            | Show log output to stdout                                       | `specstory sync --console`                                                        |
    | `--log`                | Debug output to `./specstory/debug/debug.log`                   | `specstory sync --log`                                                            |
    | `--debug`              | Enable debug level output for `--log` or `--console`            | `specstory sync --log --debug`                                                    |
    | `--silent`             | Supress all non-error output                                    | `specstory sync --silent`                                                         |
    | ` --no-cloud-sync`     | Disable cloud sync, even when logged in to SpecStory Cloud      | `specstory sync --no-cloud-sync`                                                  |
    | `--no-usage-analytics` | Disable usage analytics                                         | `specstory run cursor --no-usage-analytics`                                       |
    | `--no-version-check`   | Skip checking for newer version                                 | `specstory run codex --no-version-check`                                          |
    ...
    - **Cloud Sync:** Sessions synced to the SpecStory Cloud if logged in with `specstory login` and the `--no-cloud-sync` flag isn't specified
    - **Session Files:** Read from `` ~/.claude/projects/ and `~/.cursor/chats/ ``
    ...
    - `.specstory/.project.json` for project tracking
    ...
    Use console mode or logging to troubleshoot:
    ...
    The console and log flags show:
