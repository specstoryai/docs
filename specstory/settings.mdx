...
<Tabs>
  ...
  <Tab title="Terminal Coding Agents">
    ...
    | `run`     | Run Claude Code or Cursor CLI with auto-save to to local markdown files, and to SpecStory Cloud if logged in | `specstory run claude`<br />`specstory run cursor`<br />`specstory run codex` |
    ...
    | `-s, --session`        | Sync a specific session to markdown                             | `specstory sync cursor -s <sessionId>`                                            |
    ...
    specstory sync --console | grep "Parsing session"
    ```
