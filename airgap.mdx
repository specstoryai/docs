...
- **`!pattern`** → **Full Protection** (no add/modify/delete)
- **`~pattern`** → **Protect Existing** (cannot modify/delete existing files, but can add new ones)  
- **`pattern`** → **Allow** (changes are safe)
